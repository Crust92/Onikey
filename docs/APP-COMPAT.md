...
- `purpose` — kiểu ô nhập (IBusInputPurpose): `0` thường, `5` URL, `6` email,
  `8` mật khẩu. Onikey dùng `5` để tự bỏ gạch chân ở ô địa chỉ.
...
| Microsoft Edge — ô địa chỉ | `0x29` (lúc đầu `0x9`) | **`5`** | Surrounding Text (không gạch chân) | Edge báo capability theo hai nhịp; phải đợi nhịp thứ hai, xem `updateURLInputMode` |
...
2. **Chỉ bỏ gạch chân khi ứng dụng thật sự hỗ trợ.** Onikey kiểm tra bit
   ...
   `updateURLInputMode`). Từng có bản lùi về *Forward as commit* khi thiếu bit
   này và hậu quả là ô địa chỉ Edge nuốt phím.
